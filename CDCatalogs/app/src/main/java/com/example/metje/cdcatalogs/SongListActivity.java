package com.example.metje.cdcatalogs;
		...
		yearInput.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				if (yearInput.getText().length() <= 0 ) {
					button.setEnabled(false);
					Toast.makeText(SongListActivity.this, "Year cannot be empty", Toast.LENGTH_SHORT).show();
				} else if (Integer.parseInt(yearInput.getText().toString()) < 1000) {
					button.setEnabled(false);
					Toast.makeText(SongListActivity.this, "That's not a reasonable year", Toast.LENGTH_SHORT).show();
				} else if (Integer.parseInt(yearInput.getText().toString()) > Calendar.getInstance().get(Calendar.YEAR)) {
					button.setEnabled(false);
					Toast.makeText(SongListActivity.this, "So You've heard an album from the future uh?", Toast.LENGTH_SHORT).show();
				} else {
					button.setEnabled(true);
				}
			}
		...
		songInput.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				if (songInput.getText().length() <= 0 || songlist.contains(songInput.getText().toString())) {
					button.setEnabled(false);
					Toast.makeText(SongListActivity.this, "Name too short or exist already", Toast.LENGTH_SHORT).show();
				} else {
					button.setEnabled(true);
				}
			}
